...
function output_html($json){
	...
	foreach ($nodes['good'] as $key => $node) {
		echo '<tr><th>'.$node['object'].'</th><th>'.$node['version'].'</th><th>'.$node['ip'].':'.$node['port'].'</th><th>'.$node['os'].'</th><th>'.$node['height'].'</th><th>'.$node['info'].'</th><th>'.$node['contact'].'</th>';
	}
	echo "</table>";
	...
	foreach ($nodes['bad'] as $key => $node) {
		echo '<tr><th>'.$node['object'].'</th><th>'.$node['version'].'</th><th>'.$node['ip'].':'.$node['port'].'</th><th>'.$node['os'].'</th><th>'.$node['height'].'</th><th>'.$node['info']['details'].'</th><th>'.$node['contact'].'</th>';
	}
	echo "</table></center>";
	die();
}
